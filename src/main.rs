...
// self is a rust port of the https://github.com/iden3/circomlib/blob/master/src/evmasm.js tool to rust
struct EVMCodeGen {
    code : Vec<u8>,
...
impl EVMCodeGen {
    ...
    pub fn op_jump(&mut self, label: Option<&str>)  {
        if let Some(label) = label {
            let offset = self.ref_label(label);
            self.op_push(&offset);
        }
        self.code.push(0x56);
    }

    pub fn op_jumpi(&mut self, label: Option<&str>)  {
        if let Some(label) = label {
            let offset = self.ref_label(label);
            self.op_push(&offset);
        }
    ...
    fn ref_label(&mut self, label: &str) -> Vec<u8>{
        if let Some(loc) = self.labels.get(label) {
            ...
            self.pending_labels.entry(label.to_string()).or_insert(Vec::new()).push(self.code.len());
            vec![0,0,0]
        }
    }
...
mod test {
    ...
    fn test_call_a() {
        ...
        assert_code_eq(&b,"5b6000600060006000600061ff0b5af1600056");
    }
